...
const SENSITIVE_KEYWORDS =
  /pass(word)?|auth|credential|login|secret|NICANOR_EMAIL_PASSWORD|NICANOR_MCP_TOKEN|authorization|bearer|access[_-]?token/gi;

function collectSecretValues() {
  const secrets = [];
  const pw = process.env.NICANOR_EMAIL_PASSWORD;
  if (typeof pw === 'string' && pw.length > 0) secrets.push(pw);
  const mcpToken = process.env.NICANOR_MCP_TOKEN;
  if (typeof mcpToken === 'string' && mcpToken.length > 0) secrets.push(mcpToken);
