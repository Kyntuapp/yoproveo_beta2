...
function buildProductMetrics(productos, ofertas, progressRatio) {
  ...
  const conexionesSemana = productosSemana + ofertasSemana;
...
function filterEncuestaPayload(encuesta, pilotoCodigo, fechaInicioMedicion) {
  const matchPiloto = (row) => !pilotoCodigo || row.piloto_codigo === pilotoCodigo;

  const scores_semana_piloto = encuesta.scores_semana_piloto.filter(matchPiloto);
  const por_pregunta = encuesta.por_pregunta.filter(matchPiloto);
  const comentarios = encuesta.comentarios.filter(
    (row) =>
      matchPiloto(row) &&
      encuestaDesdeFechaValida(row.fecha_respuesta, fechaInicioMedicion)
  );
...
function buildIndiceGeneral(dimensionScores, timeline, dimensionScoresMetaFinal = null) {
...
export function computeWarRoomDashboard({
  ...
  const timeline = buildTimeline(piloto);
  const progressRatio = timeline.progress_ratio;
  const metrics = buildProductMetrics(productos, ofertas, progressRatio);

  const fechaInicioMedicion = pilotoConfig.fecha_inicio_medicion;
  ...
  const encuestaRespuestasValidas = encuestaRespuestas
    .filter(
      (r) =>
        (!piloto?.codigo || r.piloto_codigo === piloto.codigo) &&
        encuestaDesdeFechaValida(r.fecha_respuesta, fechaInicioMedicion)
    )
    .map((r) => ({
      ...r,
      semana_visual: isAntesInicioPiloto(r.fecha_respuesta, fechaInicioPiloto)
        ? 'pre-piloto'
  ...
  const preInicio = timeline.pre_inicio;
  ...
  const indiceGeneral = buildIndiceGeneral(dimensionScores, timeline, dimensionScoresMeta);
  ...
  const alertas = timeline.pre_inicio ? [] : buildAlertas(indicadores);
  ...
  return {
    ...
    configuracion: {
      ...
      encuestas: {
        estado_encuesta: 'Pendiente de control',
        modo_medicion: pilotoConfig.modo_medicion_label,
        fecha_inicio_medicion: pilotoConfig.fecha_inicio_medicion,
      },
