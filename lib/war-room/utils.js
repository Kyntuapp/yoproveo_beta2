...
export function fechaEnRango(fecha, desde, hasta) {
  ...
  if (!f || !d || !h) return false;
  return f >= d && f <= h;
}

export function isoDateToday(timeZone = 'America/Santiago') {
  return new Date().toLocaleDateString('en-CA', { timeZone });
}

/** Semanas completas del periodo (inclusive) para cumplimiento de encuestas. */
export function calcSemanasPeriodo(desde, hasta) {
  const fin = parseDate(hasta);
  if (!desde || !fin) {
    return { semanas_periodo: 1, semanas_totales: 1, semanas_transcurridas: 1 };
  }
  const { semanas_totales } = calcSemanasEncuesta(desde, hasta, fin);
  return {
    semanas_periodo: semanas_totales,
    semanas_totales,
    semanas_transcurridas: semanas_totales,
  };
}

const ISO_DATE_RE = /^\d{4}-\d{2}-\d{2}$/;

export function parsePeriodoMedicion(desdeRaw, hastaRaw) {
  const desde = String(desdeRaw || '').trim();
  const hasta = String(hastaRaw || '').trim();
  if (!ISO_DATE_RE.test(desde) || !ISO_DATE_RE.test(hasta)) {
    return null;
  }
  const d = parseDate(desde);
  const h = parseDate(hasta);
...
export function estadoValidacionTecnica() {
  return {
    id: 'tech',
