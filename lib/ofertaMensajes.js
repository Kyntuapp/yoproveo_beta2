...
import { ESTADOS_ADJUDICACION_SOLICITUD } from './estadosAdjudicacion';

const ESTADOS_ADJUDICADOS = new Set(ESTADOS_ADJUDICACION_SOLICITUD);

export { ESTADOS_ADJUDICACION_SOLICITUD };
...
export async function solicitudTieneOfertaAdjudicada(listasComprasId) {
  ...
  return (await fetchSolicitudesAdjudicadasIds([listasComprasId])).has(listasComprasId);
}
...
export async function fetchSolicitudesAdjudicadasIds(listasComprasIds) {
  const ids = [...new Set((listasComprasIds || []).filter((id) => esUuidValido(id)))];
  if (!ids.length) return new Set();
  const { data, error } = await supabase.auth.getSession();
  if (error || !data?.session?.access_token) throw new Error('Debes iniciar sesión.');
  const adjudicadas = new Set();
  for (let start = 0; start < ids.length; start += 200) {
    const response = await fetch('/api/proveedor/solicitudes-adjudicadas', {
      method: 'POST',
    ...
    if (!response.ok || !Array.isArray(result.ids)) {
      throw new Error(result.error || 'No se pudo verificar la disponibilidad.');
    }
    result.ids.forEach(id => adjudicadas.add(id));
  }
  return adjudicadas;
}
