...
import { ESTADOS_OFERTA_PENDIENTE_PAGO } from '../estadosOfertaComprador';
...
export async function getPendingOffersForBuyer(userId) {
  ...
  const { data: offers, error: offersError } = await supabaseAdmin
    ...
    .in('estado', ESTADOS_OFERTA_PENDIENTE_PAGO);
  if (offersError) throw offersError;

  const offerIds = (offers || []).map((offer) => offer.id);
  if (!offerIds.length) return [];
  const { data: paidRecords, error: paidRecordsError } = await supabaseAdmin.from('pagos')
    .select('oferta_id').in('oferta_id', offerIds).in('estado_pago', ['pagado', 'approved']);
  if (paidRecordsError) throw paidRecordsError;
  const paidOfferIds = new Set((paidRecords || []).map((payment) => String(payment.oferta_id)));

  return (offers || []).filter((offer) => !paidOfferIds.has(String(offer.id))).map((offer) => {
...
export async function createPaymentOrder(userId, provider, offerIds) {
  ...
  if (provider === 'transferencia') return createTransferOrder(userId, selected);
...
export async function createPaymentOrderFromCheckout(userId, provider, checkoutOrderId) {
  ...
  const items = checkoutItems.map((item) => {
    ...
    if (commission !== 0 || total !== amount) {
      throw new Error('Esta orden conserva un recargo antiguo. Kyntü no cobra comisión durante el MVP. Debemos corregir la orden antes de iniciar el pago.');
    }
    return {
      id: item.oferta_id,
  ...
  if (provider === 'transferencia') return createTransferOrder(userId, items, checkoutOrderId);
...
async function createTransferOrder(userId, items, checkoutId = null) {
  const { data, error } = await supabaseAdmin.rpc('crear_transferencia_kyntu', {
    p_buyer: userId, p_checkout: checkoutId,
    p_items: items.map((item) => ({ offer_id: item.id, provider_profile_id: item.proveedor_id,
      title: item.producto, amount: item.amount, commission: item.commission,
      provider_net: item.providerNet, total: item.total })),
  });
