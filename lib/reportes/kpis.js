...
export function computeReportKpis(listas, ofertas) {
  ...
  for (const oferta of ofertas) {
    ...
    if (ESTADOS_ACEPTADOS.has(estado)) {
      ...
      if (!Number.isNaN(precioOfertado)) {
        gmvPotencialRaw += precioOfertado;
      }
    }
  }
  ...
  const tasaCoberturaPct =
    listasPublicadas > 0
      ? round1(
          ((listasPublicadas - lotesSinOfertas) / listasPublicadas) * 100
        )
      : 0;
  ...
  const gmvPotencial = roundMoney(gmvPotencialRaw);
  const ingresosKyntuEstimados = roundMoney(gmvPotencialRaw * 0.03);
  ...
  return {
    ...
    finanzas_estimadas: {
      gmv_potencial: gmvPotencial,
      ingresos_kyntu_estimados: ingresosKyntuEstimados,
    },
  };
}
