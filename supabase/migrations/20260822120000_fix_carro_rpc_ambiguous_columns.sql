-- =============================================================================
-- HOTFIX — columnas ambiguas en RPCs del carro
--
-- Causa: RETURNS TABLE(... orden_id/estado/lista_id/oferta_id ...) crea OUT vars
-- que colisionan con columnas homónimas en UPDATE/SELECT sin alias.
--
-- Fix mínimo: alias de tabla + columnas calificadas. Sin cambio de reglas/firmas.
-- RPCs: confirmar_orden_checkout, cancelar_orden_checkout, revertir_adjudicacion
-- (crear_orden_checkout / obtener_orden_checkout sin ambigüedad inequívoca).
-- =============================================================================
...
BEGIN
  ...
  SELECT count(*)::integer,
         count(*) FILTER (WHERE lower(trim(coalesce(i.estado_item, ''))) = 'incluido')::integer,
  ...
  UPDATE public.ordenes_checkout_items AS i
...
BEGIN
  ...
  UPDATE public.ofertas_productos AS o
  SET
    estado = 'rechazada',
    estado_motivo = 'cancelacion_carro',
    adjudicacion_origen_id = NULL
  WHERE o.id = p_oferta_id
    AND lower(trim(coalesce(o.estado, ''))) = 'pendiente_pago';
  ...
  WITH updated AS (
    ...
    SET
      ...
      estado_motivo = NULL,
      adjudicacion_origen_id = NULL
    WHERE o.lista_id = v_lista_compras_id
      AND lower(trim(coalesce(o.estado, ''))) = 'rechazada'
      AND o.estado_motivo = 'adjudicacion_automatica'
      AND o.adjudicacion_origen_id = p_oferta_id
    RETURNING o.id
  )
  SELECT
