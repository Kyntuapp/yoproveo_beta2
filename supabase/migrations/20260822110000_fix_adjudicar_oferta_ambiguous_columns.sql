-- =============================================================================
-- HOTFIX — adjudicar_oferta: column reference "estado" is ambiguous
--
-- Causa: RETURNS TABLE(... estado text, lista_id uuid, adjudicada_en ...)
-- crea variables OUT en el scope PL/pgSQL. Las referencias no calificadas a
-- columnas de ofertas_productos (estado, lista_id) en UPDATE ... WHERE
-- colisionan con esos nombres.
--
-- Fix mínimo: alias de tabla + columnas calificadas. Sin cambio de reglas.
-- =============================================================================
...
BEGIN
  ...
  WHERE o.id = p_oferta_id
    AND lower(trim(coalesce(o.estado, ''))) = 'pendiente';
  ...
  WITH updated AS (
    ...
    WHERE o.lista_id = v_lista_compras_id
      AND o.id IS DISTINCT FROM p_oferta_id
      AND lower(trim(coalesce(o.estado, ''))) = 'pendiente'
    RETURNING o.id
  )
