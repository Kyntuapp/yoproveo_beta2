-- Instalacion completa para la base actual de Kyntu.
-- Incluye dependencias faltantes y no genera liquidaciones historicas.
...
SELECT
  to_regclass('public.provider_payouts') IS NOT NULL AS tabla_liquidaciones_lista,
  to_regprocedure('public.crear_transferencia_kyntu(uuid,uuid,jsonb)') IS NOT NULL AS iniciar_pago_listo,
  to_regprocedure('public.confirmar_transferencia_kyntu(uuid,text,bigint,uuid)') IS NOT NULL AS confirmar_abono_listo,
  to_regprocedure('public.preparar_nomina_kyntu(uuid)') IS NOT NULL AS nomina_lista;
